...
Order status is a one-way state machine: `Pending` → `Reserved` or `Pending` →
`Rejected`. See [order.md](order.md) for the entity detail.
...
| RabbitMQ (dev) / Azure Service Bus (non-dev) | Order, Inventory | `OrderCreatedEvent`, `InventoryReservedEvent`, `InventoryRejectedEvent` |
